...
def test_llm_client_import():
    ...
    try:
        from llm_client import create_llm_client  # type: ignore  # noqa: F401
...
def test_anthropic_client_creation():
    ...
    try:
        ...
        try:
            create_llm_client(provider="anthropic", api_key=None)
            print("❌ Should have raised ValueError for missing API key")
            return False
        except ValueError as e:
            print(
                f"✅ Correctly raises ValueError for missing API key: {str(e)[:50]}..."
            )
            return True
        except ImportError:
...
def test_rag_integration():
    ...
    try:
        ...
        from rag.config import RAGConfig  # type: ignore  # noqa: E402
        ...
        if chat_engine.use_llm:
            print("   LLM Status: Enabled")
        else:
            print("   LLM Status: Using fallback (no Ollama server)")
...
def test_synthetic_integration():
    ...
    try:
        ...
        from synthetic.generator import SyntheticDataGenerator  # type: ignore  # noqa: E402
        ...
        if generator._llm_available:
            ...
            print("   LLM Status: Using mock data (no Ollama server)")
