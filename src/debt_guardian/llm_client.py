...
class OllamaClient:
    ...
    def generate(
        ...
        try:
            response = self.client.chat(
                model=self.config.llm_model, messages=messages, options=options
            )
    ...
    def generate_structured(
        ...
        response_text = self.generate(full_prompt, system_prompt=system_prompt)
    ...
    def health_check(self) -> bool:
        ...
        try:
            ...
            response = self.generate(
                "Respond with 'OK' if you can read this.", temperature=0.0
            )
