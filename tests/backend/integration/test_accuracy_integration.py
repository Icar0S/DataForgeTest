...
class TestDataAccuracyIntegration:
    ...
    def test_full_workflow(self):
        ...
        assert diffs[0]["gold"] == pytest.approx(5.0)
        assert diffs[0]["target"] == pytest.approx(6.0)
    ...
    def test_duplicate_in_gold_error(self):
        ...
        gold_dup_df = pd.DataFrame(
            {"produto": ["A", "A", "B"], "preco": [10.0, 15.0, 20.0]}  # Duplicate A
        )
        ...
        # Upload TARGET
        target_df = pd.DataFrame({"produto": ["A", "B"], "preco": [10.0, 20.0]})
