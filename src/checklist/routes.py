...
from .models import template_to_dict, ItemStatus
...
def health_check():
    ...
    return jsonify({
        "status": "ok",
        "message": "Checklist service is running"
    })
...
def generate_recommendations(run_id):
    ...
    try:
        ...
        if missing_items:
            ...
            query = "Forneça recomendações para implementar os seguintes itens de QA de Big Data:\n" + "\n".join(query_parts)
            ...
            try:
                ...
                recommendations.append({
                    ...
                    "content": rec_content,
                    "sources": [r.get("metadata", {}).get("filename", "Unknown") for r in results[:3]]
                })
